...
## IAM Users
_It is an entity that represents a person or service (a user account that can be used by a service rather than human being)
_We can apply a policy which is a list of permissions which says what this user is allowed to do on AWS
_By default, users cannot do anything in the account. Users need a policy in order to do something in the account.
_Up to 5000 users per AWS account.
_Each user account will have a friendly name and an Amazon Resource Name (ARN) which uniquely identifies the user across AWS.
